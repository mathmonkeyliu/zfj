...
class ID3Agent:
    """
    Online greedy ID3 (per-step choose the unshot cell with maximal information gain),
    ...
    @staticmethod
    def from_layouts(layouts: list[dict[str, Any]]) -> "ID3Agent":
        outcomes, label_ids, labels = build_outcome_table(layouts)
        return ID3Agent(outcomes=outcomes, label_ids=label_ids, labels=labels)

    def _best_action(self, cand_idx: np.ndarray, unshot: np.ndarray) -> int:
        """
        Choose feature maximizing IG(label; outcome(feature)).
        """
    ...
    def play_one(self, env: BombPlanesEnv, *, layout: dict[str, Any], max_steps: int = 500) -> int:
        ...
        while heads_hit < 3 and steps < max_steps:
            # If only one possible head configuration remains, finish by shooting those heads.
            ...
            # Filter candidates consistent with observed outcome at this cell.
            col = self.outcomes[cand_idx, int(a)]
            cand_idx = cand_idx[col == obs_v]

            if cand_idx.size == 0:
                # Should not happen if layouts universe matches env layout file,
                # but guard to avoid crash.
                break
