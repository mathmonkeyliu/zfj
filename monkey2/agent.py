...
from monkey2.search import SearchContext, make_progress, _pick_topk_actions
from monkey2.state_hash import state_hash_hex
...
class MonkeyAgent:
    """
    Monkey2 Agent.
    Uses precomputed Minimax tree. Falls back to ID3 (Greedy) if state not found.
    """
    ...
    def choose_action(
        ...
        *,
        ...
        # 2. Fallback to ID3 (Greedy Top 1)
        # We enforce top_k=1 locally for fallback
        fallback_cfg = MonkeyConfig(
            top_k=1,
            small_candidates_threshold=0, # disable boost
            top_k_when_small_candidates=1,
            symmetry_enabled=self.cfg.symmetry_enabled,
            symmetry_gain_round_ndigits=self.cfg.symmetry_gain_round_ndigits,
            symmetry_consider_state=self.cfg.symmetry_consider_state,
            progress_enabled=False
        )

        sym = SymmetryGroup.build()
        progress = make_progress(fallback_cfg, enabled=False)
        ctx = SearchContext(
        ...
        # Pick best action (ID3 style)
        acts = _pick_topk_actions(ctx, cand_idx, unshot_actions, dict(shots_use))
        if acts:
            return int(acts[0])
            
        # Fallback if no actions found (should not happen)
